...
void draw_fuel_limit_circle (int cx, int cy)
{
	...
	gfx_draw_circle (cx, cy, radius, col_green);

	gfx_draw_colour_line (cx, cy - cross_size, cx, cy + cross_size,GFX_COL_WHITE);
	gfx_draw_colour_line (cx - cross_size, cy, cx + cross_size, cy,GFX_COL_WHITE);
}
...
int calc_distance_to_planet (struct galaxy_seed from_planet, struct galaxy_seed to_planet)
{
	...
	light_years = __ieee754_sqrt(dx + dy);
	light_years *= 4;
...
void display_short_range_chart (void)
{
	...
	gfx_draw_colour_line (0, 36/2, 255, 36/2,GFX_COL_WHITE);
	...
	for (i = 0; i < 256; i++)
	{
		...
		if (row_used[row] == 0)
		{
			...
			gfx_display_text ((px * 2) + 8, 2 * (row * 8 - 5), planet_name);
		}
		...
		/* JDW - The carry flag left over is because it saves a CLC instruction and 1 byte of code when creating the circle size */
		/* the actual code would be LDA gfx.f, AND #1, ADC #2, the carry flag was left over from the name routine and doesn't break the game in any other way */
		/* Just means we have 2,3 or 4 pixel size stars on the map instead of 2 or 3 */
...
void display_galactic_chart (void)
{
	...
	gfx_draw_colour_line (0, 36/2, 255, 36/2,GFX_COL_WHITE);
	gfx_draw_colour_line (0, (36+258) / 2, 255, (36+258) /2,GFX_COL_WHITE);
	...
	for (i = 0; i < 256; i++)
	{
		...
		if ((glx.e | 0x50) < 0x90)
			gfx_plot_pixel (px, py+1, GFX_COL_WHITE);
...
void display_commander_status (void)
{
	...
	gfx_display_colour_text (16, 58, "Present System:", col_green);
	...
	if (!witchspace)
	{
		...
		gfx_display_text (280, 58, str);
	}

	gfx_display_colour_text (16, 74, "Hyperspace System:", col_green);
	name_planet (planet_name, hyperspace_planet);
	capitalise_name (planet_name);
	sprintf (str, "%s", planet_name);
	gfx_display_text (280, 74, str);
	...
	gfx_display_colour_text (16, 90, "Condition:", col_green);
	gfx_display_text (280, 90, condition_txt[condition]);

	sprintf (str, "%d.%d Light Years", cmdr.fuel / 10, cmdr.fuel % 10);
	gfx_display_colour_text (16, 106, "Fuel:", col_green);
	gfx_display_text (90, 106, str);

	sprintf (str, "%d.%d Cr", cmdr.credits / 10, cmdr.credits % 10);
	gfx_display_colour_text (16, 122, "Cash:", col_green);
	gfx_display_text (90, 122, str);
	...
	gfx_display_colour_text (16, 138, "Legal Status:", col_green);
	gfx_display_text (200, 138, str);

	for (i = 0; i < NO_OF_RANKS; i++)
		if (cmdr.score >= rating[i].score)
			strcpy (str, rating[i].title);
	
	gfx_display_colour_text (16, 154, "Rating:", col_green);
	gfx_display_text (120, 154, str);

	gfx_display_colour_text (16, 186, "EQUIPMENT:", col_green);
...
void highlight_stock (int i)
{
	...
	{
		y = (hilite_item * 16 + 56);
		gfx_clear_area (2, y/2, 252, 8);
		display_stock_price (hilite_item);		
	}

	y = (i * 16 + 56);
	
	gfx_draw_rectangle (2, y/2, 252, 8, GFX_COL_DARK_RED);
...
void display_market_prices (void)
{
	...
	gfx_display_colour_text (16,  40, "PRODUCT", col_green);
	gfx_display_colour_text (166, 40, "UNIT", col_green);
	gfx_display_colour_text (246, 40, "PRICE", col_green);
	gfx_display_colour_text (320, 40, "F/SALE", col_green);
	gfx_display_colour_text (430, 40, "HOLD", col_green);
...
void display_inventory (void)
{
	...
	sprintf (str, "%d.%d Light Years", cmdr.fuel / 10, cmdr.fuel % 10);
	gfx_display_colour_text (16, 50, "Fuel:", col_green);
	gfx_display_text (90, 50, str);

	sprintf (str, "%d.%d Cr", cmdr.credits / 10, cmdr.credits % 10);
	gfx_display_colour_text (16, 66, "Cash:", col_green);
	gfx_display_text (90, 66, str);
...
void list_equip_prices (void)
{
	...
	gfx_clear_area (2, 55/2, 252, (380-55)/2);
