...
void clear_universe (void)
{
	...
	memset(ship_count,0,sizeof(ship_count));
...
int add_new_ship (int ship_type, int x, int y, int z, struct vector *rotmat, int rotx, int rotz)
{
	...
	for (i = 0; i < MAX_UNIV_OBJECTS; i++)
	{
		if (universe[i].type == 0)
		{
			...
			universe[i].distance = __ieee754_sqrt(x*x + y*y + z*z);
			...
			universe[i].flags = (ship_type < 0) ? 0 : initial_flags[ship_type];
...
void remove_ship (int un)
{
	...
	{
		info_message("Uh Oh! Stella Body removed.");
	}
}
