...
void move_univ_object (struct univ_object *obj)
{
	...
	obj->distance = __ieee754_sqrt (x*x + y*y + z*z);
...
void update_altitude (void)
{
	...
	x = fabs(universe[planet].location.x);
	y = fabs(universe[planet].location.y);
	z = fabs(universe[planet].location.z);
...
void update_cabin_temp (void)
{
	...
	x = abs((int)universe[sun].location.x);
	y = abs((int)universe[sun].location.y);
	z = abs((int)universe[sun].location.z);
...
void update_scanner (void)
{
	...
	for (i = 0; i < MAX_UNIV_OBJECTS; i++)
	{
		...
		switch (universe[i].type)
		{
			...
			case SHIP_DODEC:
			case SHIP_CORIOLIS:
				colour = nula ? GFX_COL_GREEN_1 : GFX_COL_GREEN_2;
				break;
		...
		gfx_draw_colour_line ((x1+2)/2, y2/2,   (x1-3)/2, y2/2, colour);
		gfx_draw_colour_line ((x1+2)/2, (y2+1)/2, (x1-3)/2, (y2+1)/2, colour);
		gfx_draw_colour_line ((x1+2)/2, (y2+2)/2, (x1-3)/2, (y2+2)/2, colour);
		gfx_draw_colour_line ((x1+2)/2, (y2+3)/2, (x1-3)/2, (y2+3)/2, colour);


		gfx_draw_colour_line (x1/2,   y1/2, x1/2,   y2/2, colour);
		gfx_draw_colour_line ((x1+1)/2, y1/2, (x1+1)/2, y2/2, colour);
		gfx_draw_colour_line ((x1+2)/2, y1/2, (x1+2)/2, y2/2, colour);
	}
}
...
void update_compass (void)
{
	...
	if (dest.z < 0)
	{
		gfx_draw_sprite (IMG_RED_DOT, compass_x/2, compass_y/2);
	}
	else
	{
		gfx_draw_sprite (IMG_GREEN_DOT, compass_x/2, compass_y/2);
	}
...
void display_speed (void)
{
	...
	len = ((flight_speed * 32) / myship.max_speed) - 1;
...
void display_dial_bar (int len, int x, int y)
{
	...
	gfx_draw_colour_line (x, y + 384/2, x + len, y + 384/2, GFX_COL_GOLD);
	i++;
	gfx_draw_colour_line (x, y + i + 384/2, x + len, y + i + 384/2, GFX_COL_GOLD);
	
	for (i = 2; i < 5; i++)
		gfx_draw_colour_line (x, y + i + 384/2, x + len, y + i + 384/2, GFX_COL_YELLOW_1);

	gfx_draw_colour_line (x, y + i + 384/2, x + len, y + i + 384/2, GFX_COL_DARK_RED);
}
...
{
	if (front_shield > 7)
		display_dial_bar (front_shield / 8, 32/2, 7/2);

	if (aft_shield > 7)
		display_dial_bar (aft_shield / 8, 32/2, 23/2);
}


void display_altitude (void)
{
	if (myship.altitude > 7)
		display_dial_bar (myship.altitude / 8, 32/2, 92/2);
}

void display_cabin_temp (void)
{
	if (myship.cabtemp > 3)
		display_dial_bar (myship.cabtemp / 8, 32/2, 60/2);
}
...
{
	if (laser_temp > 0)
		display_dial_bar (laser_temp / 8, 32/2, 76/2);
}
...
void display_energy (void)
{
	...
	if (e4 > 0)
		display_dial_bar (e4/2, 416/2, 61/2);

	if (e3 > 0)
		display_dial_bar (e3/2, 416/2, 79/2);

	if (e2 > 0)
		display_dial_bar (e2/2, 416/2, 97/2);

	if (e1 > 0)
		display_dial_bar (e1/2, 416/2, 115/2);
}
...
void display_flight_roll (void)
{
	...
	pos = sx - ((flight_roll * 14) / myship.max_roll);
	pos += 16;

	for (i = 0; i < 2; i++)
	{
		gfx_draw_colour_line (pos + i, sy, pos + i, sy + 3, GFX_COL_GOLD);
	}
}
...
void display_flight_climb (void)
{
	...
	pos = sx + ((flight_climb * 14) / myship.max_climb);
	pos += 16;
...
{
	if (cmdr.fuel > 0)
		display_dial_bar ((cmdr.fuel * 32) / myship.max_fuel, 32/2, 44/2);
}
...
{
	gfx_set_clip_region (0, 0, 256, 256);
	gfx_draw_scanner();
	...
	if (ship_count[SHIP_CORIOLIS] || ship_count[SHIP_DODEC])
		gfx_draw_sprite (IMG_BIG_S, 387/2, 490/2);

	if (ecm_active)
		gfx_draw_sprite (IMG_BIG_E, 115/2, 490/2);
}
...
void display_hyper_status (void)
{
	...
	{
		gfx_clear_area (3, 3, 20/2, 29/2);
		gfx_display_text (3, 3, str);
	}
}
...
void complete_hyperspace (void)
{
	...
	planet = add_new_ship (SHIP_PLANET, px, py, pz, rotmat, 0, 0);

	pz = -(((docked_planet.d & 7) | 1) << 16);
	px = ((docked_planet.f & 3) << 16) | ((docked_planet.f & 3) << 8);

	sun = add_new_ship (SHIP_SUN, px, py, pz, rotmat, 0, 0);
...
void jump_warp (void)
{
	...
	if ((universe[planet].distance < 75001) || (universe[sun].distance < 75001))
	{
		info_message ("Mass Locked");
		return;
	}

	if (universe[planet].distance < universe[sun].distance)
		jump = universe[planet].distance - 75000;
	else
		jump = universe[sun].distance - 75000;	
...
void launch_player (void)
{
	...
	generate_landscape(docked_planet.a * 251 + docked_planet.b);
	set_init_matrix (rotmat);
	planet = add_new_ship (SHIP_PLANET, 0, 0, 65536, rotmat, 0, 0);
	sun = -1;
