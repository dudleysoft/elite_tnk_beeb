...
extern char *remapPath(const char *path);

void write_config_file (void)
{
	FILE *fp;
	
	fp = fopen (remapPath("newkind.cfg"), "w");
	if (fp == NULL)
		return;
...
void read_config_file (void)
{
	...
	fp = fopen (remapPath("newkind.cfg"), "r");
	if (fp == NULL)
		return;
	...
	// Force this to always be off, it doesn't look great with the downscaling, so I'm removing it completely.
	anti_alias_gfx = 0;
