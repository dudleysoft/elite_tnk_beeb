...
int get_time()
{
	// Buffer to hold the 5 bytes of the system timer
	int out[2];
	// Read timer
	_swi(OS_Word,_INR(0,1),1,out);
	// Return he lowest 32 bits of the timer value
	return out[0];
}
...
void draw_cross (int cx, int cy)
{
	if (current_screen == SCR_SHORT_RANGE)
	{
		gfx_set_clip_region (2, 38/2, 254, 338/2);
		// xor_mode (TRUE);
		gfx_draw_xor_line (cx - 16, cy, cx + 16, cy, GFX_COL_RED);
		gfx_draw_xor_line (cx, cy - 16, cx, cy + 16, GFX_COL_RED);
		gfx_draw_xor_line (cx^1, cy - 16, cx^1, cy + 16, GFX_COL_RED);
		// xor_mode (FALSE);
		gfx_set_clip_region (2, 1, 252, 191);
		return;
	}
	
	if (current_screen == SCR_GALACTIC_CHART)
	{
		gfx_set_clip_region (2, 37/2, 254, 293/2);
		// xor_mode (TRUE);
		gfx_draw_xor_line (cx - 8, cy, cx + 8, cy, GFX_COL_RED);
		gfx_draw_xor_line (cx, cy - 8, cx, cy + 8, GFX_COL_RED);
		gfx_draw_xor_line (cx^1, cy - 8, cx^1, cy + 8, GFX_COL_RED);
		// xor_mode (FALSE);
...
void draw_laser_sights(void)
{
	...
	if (laser)
	{
		...
		gfx_draw_colour_line (x1-2, y1, x1-2, y2, GFX_COL_GREY_1); 
    ...
    int keyasc;
	 ...
	 if (docked &&
	     ((current_screen == SCR_MARKET_PRICES) ||
		  ...
		  {
			// Slow down updates when docked and in an info screen
			beebScreen_VSync();
	 		kbd_read_key();
		  }
...
void save_commander_screen (void)
{
	...
	sprintf (path, "save/%s",cmdr.name);
	// strcat (path,);
	
	okay = gfx_request_file ("Save Commander", path, "save");
	...
	if (rv)
	{
		gfx_display_centre_text (340, "Error Saving Commander!", 140, GFX_COL_GOLD);
		return;
	}
	gfx_clear_text_area();
	gfx_display_centre_text (340, "Commander Saved.", 140, GFX_COL_GOLD);

	set_commander_name (&path[5]);
	saved_cmdr = cmdr;
...
{
	int x,y;
	_swi(OS_Byte,_INR(0,2)|_OUTR(1,2),129,0,0,&x,&y);
	if (y==27)
	{
		return 27;
	}
...
void load_commander_screen (void)
{
	...
	gfx_draw_colour_line (0, 36, 511, 36,GFX_COL_WHITE);
	gfx_update_screen();
	
	
	strcpy (path, "save/jameson");
	
	rv = gfx_request_file ("Load Commander", path, "load");
...
void run_game_over_screen()
{
	...
	// sprintf(message_string,"Game Over = %d",game_over);
...
void display_break_pattern (void)
{
	...
	for (i = 0; i < 20; i++)
	{
		gfx_draw_circle (256/2, 192/2, (30 + i * 15)/2, GFX_COL_WHITE);
		gfx_update_screen();
	}	
...
int main(int argc,char *argv[])
{
	...
	for(int i=1;i<argc;++i)
	{
		...
		{
			printf("Using VideoNULA.\n");
			nula = 1;
			col_green = GFX_COL_GREEN_1;
		}
		// Commander name
		if ((strcmp(argv[i],"-C")==0)||(strcmp(argv[i],"-c")==0))
		{
			...
			if (i==argc)
			{
				printf("No valid commander name specified.\n");
			}
			else
			{
				// Copy the commander name
				strncpy(saved_cmdr.name,argv[i],31);
				saved_cmdr.name[31]=0;
				strncpy(easy_cmdr.name,argv[i],31);
				easy_cmdr.name[31]=0;
				printf("New commander name: %s\n",saved_cmdr.name);
			}
		}
		if ((strcmp(argv[i],"-e")==0) ||(strcmp(argv[i],"-E")==0))
		{
			printf("Using Easy start commander.\n");
			memcpy(&saved_cmdr,&easy_cmdr,sizeof(easy_cmdr));
		}
	}
	if (!nula)
	{
		strcpy(scanner_filename,"scanbbc.bmp");
	}
