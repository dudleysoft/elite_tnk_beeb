...
void display_setting_item (int item)
{
	...
	gfx_display_colour_text (x + 256, y, setting_list[item].value[v], GFX_COL_WHITE);
}
...
void highlight_setting (int item)
{
	...
	if ((hilite_item != -1) && (hilite_item != item))
	{
		...
		{
			x = (GFX_X_CENTRE - (OPTION_BAR_WIDTH / 4)) * 2;
			y = ((NUM_SETTINGS + 1)) * 24 + 96 + 32;
		...
		gfx_clear_area (x/2, y/2, width/2, OPTION_BAR_HEIGHT/2);
	...
	gfx_draw_rectangle (x/2, y/2, width/2, OPTION_BAR_HEIGHT/2, GFX_COL_DARK_RED);
...
void highlight_option (int i)
{
	...
	{
		x = GFX_X_CENTRE - (OPTION_BAR_WIDTH / 4);
		y = (384 - (30 * NUM_OPTIONS)) / 2;
		y += hilite_item * 30;
		gfx_clear_area (x, y/2, OPTION_BAR_WIDTH/2, OPTION_BAR_HEIGHT/2);
	...
	gfx_draw_rectangle (x, y/2, OPTION_BAR_WIDTH/2, OPTION_BAR_HEIGHT/2,
						GFX_COL_DARK_RED);
